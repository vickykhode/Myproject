...
public class UpdateEmployee extends JFrame implements ActionListener{
    ...
    UpdateEmployee(String empId1){
		...
		labelempId1 = new JLabel();
		labelempId1.setBounds(200,400,150,30);
		labelempId1.setFont(new Font("serif",Font.PLAIN,20));
		add(labelempId1);
		
		try {
			Conn c = new Conn();
			String query = "select * from employee where empId1= '"+empId1+"'";
			...
			while(rs.next()) {
				...
				labelempId1.setText(rs.getString("empId1"));
			}
		}catch(Exception e) {
			e.printStackTrace();
		}
	...
	public void actionPerformed(ActionEvent ae) {
		if(ae.getSource()==add) {
			...
			try {
				Conn conn = new Conn();
				String query = "update employee set fname='"+fname+"',salary='"+salary+"',address='"+address+"',phone='"+phone+"',email='"+email+"',education='"+education+"',designation='"+designation+"'where empId1='"+empId1+"'";
